<?php
// Register 'Nomad Recent Posts' widget
add_action( 'widgets_init', 'init_wp_nomad_recent_posts' );

function init_wp_nomad_recent_posts() { return register_widget('wp_nomad_recent_posts'); }

class wp_nomad_recent_posts extends WP_Widget {
	/** constructor */
	function __construct() {
		// Instantiate the parent object
		parent::__construct( false, __( 'Nomad Recent Post', 'nomad' ) );
	}
	...
	function widget( $args, $instance ) {
		...
		// Widget options
		$title 	 = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Nomad Recent Post', 'nomad' ) : $instance['title'], $instance, $this->id_base ); // Title
	    $types   = 'post';
        ...
        <?php
			the_post_thumbnail('thumbnail', array('class' => 'alignleft widget-thumb'));
		?>
        	</a>
        <?php
		} ?>
