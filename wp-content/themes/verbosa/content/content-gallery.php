...
$options = verbosa_get_theme_options();
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); cryout_schema_microdata('article'); ?>>
	...
	<header class="entry-header">
		...
		<h2 class="entry-title" <?php cryout_schema_microdata('entry-title'); ?>>
			<span class="entry-format"><i class="icon-camera" title="<?php _e( 'Gallery', 'verbosa' ); ?>"></i></span>
	...
	<div class="entry-content" <?php cryout_schema_microdata('entry-content'); ?>>
		<?php the_content();
		wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( 'Pages:', 'verbosa' ) . '</span>', 'after' => '</div>' ) ); ?>
	</div><!-- .entry-content -->
